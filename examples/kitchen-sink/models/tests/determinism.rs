//! Determinism + snapshot gate for the kitchen-sink example
//! (main spec §50 tests 38–39, end-to-end over the union document).
//!
...
//! committed files (`--types-path kitchen_sink_models`). No tempfile crate:
...
//!
//! Snapshot regeneration: `KITCHEN_SINK_GENERATED_UPDATE=1 cargo test -p
//! kitchen-sink-models` rewrites the committed files instead of comparing
...
const TYPES_PATH: &str = "kitchen_sink_models";
...
impl TempDir {
    fn new(tag: &str) -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "kitchen-sink-determinism-{}-{id}-{tag}",
            std::process::id()
        ));
...
fn generated_artifacts_are_deterministic_and_match_commit() {
    if std::env::var("KITCHEN_SINK_GENERATED_UPDATE").as_deref() == Ok("1") {
        ...
        println!(
            "KITCHEN_SINK_GENERATED_UPDATE=1: rewrote committed artifacts under {}",
    ...
    for (index, (relative_dir, name)) in ARTIFACTS.iter().enumerate() {
        ...
        let committed = fs::read_to_string(&committed_path).unwrap_or_else(|err| {
            panic!(
                "read committed {name} at {}: {err}; run with \
                     KITCHEN_SINK_GENERATED_UPDATE=1 cargo test -p \
                     kitchen-sink-models",
        ...
        assert_eq!(
            ...
            "{name}: regenerated artifact differs from the committed file at \
             {}; run with KITCHEN_SINK_GENERATED_UPDATE=1 to refresh",
            committed_path.display()
        );
    }
}
