...
//! kitchen-sink` rewrites the committed files instead of comparing (the same
//! update-switch convention as OPENAPI_SNAPSHOT_UPDATE in
...
fn generated_artifacts_are_deterministic_and_match_commit() {
    if std::env::var("KITCHEN_SINK_GENERATED_UPDATE").as_deref() == Ok("1") {
        ...
        println!(
            "KITCHEN_SINK_GENERATED_UPDATE=1: rewrote committed artifacts in {}",
        ...
        let committed = fs::read_to_string(generated_dir().join(name)).unwrap_or_else(|err| {
            panic!(
                "read committed {name}: {err}; run with \
                     KITCHEN_SINK_GENERATED_UPDATE=1 cargo test -p kitchen-sink"
            )
        });
        assert_eq!(
            first[index],
