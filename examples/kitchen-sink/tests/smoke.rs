...
//! demo application's generated router on an ephemeral loopback port over
//! real TCP, drives the identical full-operation sweep the client binary
//! runs, and ASSERTS on the outcome: every step passes and every documented
//! operation was exercised the expected number of times.
//!
//! Gated behind `#[ignore]` so plain `cargo test` stays hermetic and fast;
//! run with `cargo test -p kitchen-sink -- --ignored`.

use std::net::SocketAddr;
use std::sync::Arc;

use kitchen_sink::demo::{self, REQUIRED_OPERATIONS};
...
async fn smoke_round_trip() {
    ...
    let failures: Vec<&demo::SweepStep> = steps.iter().filter(|step| !step.ok).collect();
    assert!(
        failures.is_empty(),
