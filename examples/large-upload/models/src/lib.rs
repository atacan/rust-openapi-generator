//! Shared OpenAPI schema surface for the large-upload example
...
//! UNMODIFIED into sibling modules so `views`' (empty here) module shape
//! stays uniform with the kitchen-sink example. `tests/determinism.rs`
//! re-runs the pipeline and proves all three example crates' committed
//! artifacts stay byte-stable (main spec §50 test 39).
//!
