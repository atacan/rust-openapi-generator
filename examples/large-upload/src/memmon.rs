...
//! task for the client side ([`ProgressTask`]) and an inline tick helper for
//! server handlers ([`ByteProgress`]) — so every printed line pairs bytes
//! transferred with the CURRENT rss, making flatness observable live.
