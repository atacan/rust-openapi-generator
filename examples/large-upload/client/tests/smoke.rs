...
//!   identical transfer sweep this crate's binary runs at a small size,
...
//!
//! The routers come from the server crate as a dev-dependency; the transfer
//! driver is this crate's own [`transfers`] module.
//!
//! Gated behind `#[ignore]` so plain `cargo test` stays hermetic and fast;
//! run with `cargo test -p large-upload-client -- --ignored`.
...
async fn assert_transfers_clean(base_url: &str, work_dir: &std::path::Path) {
    let client = transfers::build_client(base_url).expect("client builds");
    let (steps, report) = transfers::run_transfers(&client, work_dir, 8).await; // 8 MiB
    ...
    let failures: Vec<&transfers::StepOutcome> = steps.iter().filter(|step| !step.ok).collect();
    assert!(
        failures.is_empty(),
...
#[tokio::test]
#[ignore = "end-to-end smoke over real TCP; run with cargo test -p large-upload-client -- --ignored"]
...
#[tokio::test]
#[ignore = "end-to-end proxy-chain smoke over real TCP; run with cargo test -p large-upload-client -- --ignored"]
async fn smoke_proxy_chain() {
    ...
    let backend_address = spawn_router(app::demo_router(Arc::clone(&backend_app)));
    ...
    let frontend_address = spawn_router(app::demo_router(Arc::clone(&frontend_app)));
