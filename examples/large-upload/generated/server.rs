...
use super::models::{ProblemDetails, UploadReceipt};
...
/// Documented outcomes for `put_blob` (main spec §8/§13): exhaustive match required; deliberately not `#[non_exhaustive]` (§47).
#[derive(Debug)]
pub enum PutBlobResponse {
    /// HTTP 201 Created.
    Created201(UploadReceipt),
    /// HTTP 400 BadRequest.
    BadRequest400(ProblemDetails),
}

/// Bounded encoder for [`PutBlobResponse`] (main spec §8 Output B, §41): JSON/text serialize under `structured_encode_bytes`; overflow discards partial output, fires the hook, and emits a fixed empty 500 (§34.1). Range/default statuses validate their carried status (§48).
...
impl ::axum::response::IntoResponse for PutBlobResponse {
    fn into_response(self) -> ::axum::response::Response {
        self.into_response_with_limits(&BodyLimits::process_default(), &NoOpEncodeOverflowHook)
    }
}

/// Documented outcomes for `put_audio_track` (main spec §8/§13): exhaustive match required; deliberately not `#[non_exhaustive]` (§47).
#[derive(Debug)]
pub enum PutAudioTrackResponse {
    /// HTTP 201 Created.
    Created201(UploadReceipt),
    /// HTTP 400 BadRequest.
    BadRequest400(ProblemDetails),
...
/// Bounded encoder for [`PutAudioTrackResponse`] (main spec §8 Output B, §41): JSON/text serialize under `structured_encode_bytes`; overflow discards partial output, fires the hook, and emits a fixed empty 500 (§34.1). Range/default statuses validate their carried status (§48).
impl PutAudioTrackResponse {
    /// Encodes the documented outcome with the configured limits.
    pub fn into_response_with_limits(
        self,
        limits: &BodyLimits,
...
impl ::axum::response::IntoResponse for PutAudioTrackResponse {
    fn into_response(self) -> ::axum::response::Response {
        self.into_response_with_limits(&BodyLimits::process_default(), &NoOpEncodeOverflowHook)
    }
}
...
pub trait Api: Send + Sync + 'static {
    /// `PUT` `/blobs/{id}`.
    /// Operation `putBlob`.
    async fn put_blob(&self, id: String, body: ::axum::body::Body) -> PutBlobResponse;

    /// `PUT` `/audio/{id}`.
    /// Operation `putAudioTrack`.
    async fn put_audio_track(&self, id: String, body: ::axum::body::Body) -> PutAudioTrackResponse;
}
...
/// Route handler for `PUT` `/blobs/{id}` (main spec §38): identity-only content coding, parameter decoding, the §28 Content-Type state machine, and bounded collection all run before the application observes the request; every failure returns a `ProtocolRejection` outside the documented enum (§39 rule 1).
async fn route_put_blob(
    ...
    let response = api.put_blob(id, request_body).await;
    Ok(response.into_response_with_limits(&limits, hook))
}

/// Route handler for `PUT` `/audio/{id}` (main spec §38): identity-only content coding, parameter decoding, the §28 Content-Type state machine, and bounded collection all run before the application observes the request; every failure returns a `ProtocolRejection` outside the documented enum (§39 rule 1).
async fn route_put_audio_track(
    ::axum::extract::State(__state): ::axum::extract::State<ServerState>,
    ::axum::extract::Path(__path): ::axum::extract::Path<HashMap<String, String>>,
    __headers: ::http::HeaderMap,
    body: ::axum::body::Body,
    ...
    let id: String = expect_text(decoded, "id")?;
    let parsed = parse_single_content_type(&__headers)?;
    let request_body = match classify_request_entry(parsed.as_ref(), &["audio/wav"]) {
        RequestEntryMatch::AbsentContentType => {
            let (presence, _replay) =
                detect_body_presence(body.into_data_stream(), limits.peek_buffer_bytes).await;
            if matches!(presence, BodyPresence::Empty) {
    ...
    let api = __state.api.as_ref();
    let response = api.put_audio_track(id, request_body).await;
    Ok(response.into_response_with_limits(&limits, hook))
}
...
pub fn router(
    ...
    ::axum::Router::new()
        .route("/blobs/{id}", ::axum::routing::put(route_put_blob))
        .route("/audio/{id}", ::axum::routing::put(route_put_audio_track))
        .with_state(state)
}
