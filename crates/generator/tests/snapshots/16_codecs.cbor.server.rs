...
use super::models::{CborState, JsonPing, ProblemDetails};
...
impl PutCborStateResponse {
    ...
    pub fn into_response_with_limits(
        ...
        match self {
            Self::Ok200(value) => cbor_encode_limited(
                ::http::StatusCode::OK,
                "application/cbor",
                &value,
                limits,
...
pub trait Api: Send + Sync + 'static {
    ...
    async fn put_cbor_state(&self, body: CborState) -> PutCborStateResponse;
...
async fn route_put_cbor_state(
    ...
    let request_body = match classify_request_entry(parsed.as_ref(), &["application/cbor"]) {
        ...
        RequestEntryMatch::Entry(0) => {
            ...
            let value: CborState = cbor_decode_body(&bytes)?;
            value
        }
        RequestEntryMatch::Entry(_) => unreachable!("request entry index out of range"),
    };
...
pub fn router(
    ...
    ::axum::Router::new()
        ...
        .route(
            "/cbor/state",
            ::axum::routing::put(route_put_cbor_state).layer(
...
/// Typed `cbor` request-body decode from ALREADY-bounded bytes (main spec §45, D-impl-codec-plugins): the §28 Content-Type gate and the `structured_request_bytes` collection ran BEFORE this parse.
...
fn cbor_decode_body<T>(bytes: &[u8]) -> Result<T, ProtocolRejection>
where
...
/// Bounded `cbor` response encoding (main spec §45/§34/§41); the literal keeps distinct types such as application/xml separate from text/xml.
