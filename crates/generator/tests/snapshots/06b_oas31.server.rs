...
#[derive(Debug)]
pub enum GetModernResponse {
    /// HTTP 200 Ok.
    Ok200(ModernEnvelope),
}

/// Bounded encoder for [`GetModernResponse`] (main spec §8 Output B, §41): JSON/text serialize under `structured_encode_bytes`; overflow discards partial output, fires the hook, and emits a fixed empty 500 (§34.1). Range/default statuses validate their carried status (§48).
impl GetModernResponse {
...
impl ::axum::response::IntoResponse for GetModernResponse {
...
pub trait Api: Send + Sync + 'static {
    /// `GET` `/modern`.
    /// Operation `getModern`.
    async fn get_modern(&self) -> GetModernResponse;
}
...
/// Route handler for `GET` `/modern` (main spec §38): identity-only content coding, parameter decoding, the §28 Content-Type state machine, and bounded collection all run before the application observes the request; every failure returns a `ProtocolRejection` outside the documented enum (§39 rule 1).
async fn route_get_modern(
    ...
    let response = api.get_modern().await;
