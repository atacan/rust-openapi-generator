...
use super::models::{JsonPing, MsgPackEvent, ProblemDetails};
...
/// Payload for status 201 of `create_xml_document` (main spec §32): the body streams verbatim; typed documented-header fields ride beside it where documented (main spec §15/§32).
#[derive(Debug)]
pub struct CreateXmlDocument201 {
...
pub enum CreateXmlDocumentResponse {
    /// HTTP 201 Created.
    Created201(CreateXmlDocument201),
    /// HTTP 400 BadRequest.
    ...
    pub fn into_response_with_limits(
        ...
        match self {
            Self::Created201(wrapper) => {
                stream_response(::http::StatusCode::CREATED, "application/xml", wrapper.body)
            }
...
impl PostMsgPackEventResponse {
    ...
    pub fn into_response_with_limits(
        ...
        match self {
            Self::Ok200(value) => msgpack_encode_limited(
                ::http::StatusCode::OK,
                "application/msgpack",
                &value,
                limits,
...
pub trait Api: Send + Sync + 'static {
    ...
    async fn create_xml_document(&self, body: ::axum::body::Body) -> CreateXmlDocumentResponse;
    ...
    async fn post_msg_pack_event(&self, body: MsgPackEvent) -> PostMsgPackEventResponse;
...
pub fn router(
    ...
    ::axum::Router::new()
        .route(
            "/xml/documents",
            ::axum::routing::post(route_create_xml_document),
        )
        .route("/cbor/state", ::axum::routing::put(route_put_cbor_state))
        .route(
            "/msgpack/events",
            ::axum::routing::post(route_post_msg_pack_event).layer(
...
/// Typed `msgpack` request-body decode from ALREADY-bounded bytes (main spec §45, D-impl-codec-plugins): the §28 Content-Type gate and the `structured_request_bytes` collection ran BEFORE this parse.
...
fn msgpack_decode_body<T>(bytes: &[u8]) -> Result<T, ProtocolRejection>
where
    T: serde::de::DeserializeOwned,
{
    from_slice(bytes).map_err(|_| malformed_body("malformed body"))
}

/// Bounded `msgpack` response encoding (main spec §45/§34/§41); the literal keeps distinct types such as application/xml separate from text/xml.
