...
/// Documented representations for status 200 of `get_report` (main spec §11): the router selects through Content-Type matching (§28).
#[derive(Debug)]
...
/// Documented representations for status 400 of `get_report` (main spec §11): the router selects through Content-Type matching (§28).
#[derive(Debug)]
...
/// Bounded encoder for [`GetReportResponse`] (main spec §8 Output B, §41): JSON/text serialize under `structured_encode_bytes`; overflow discards partial output, fires the hook, and emits a fixed empty 500 (§34.1). Range/default statuses validate their carried status (§48).
    ...
    pub fn into_response_with_limits(
        ...
        match self {
            Self::Ok200(content) => match content {
                GetReport200Content::Json(value) => encode_json_limited(
                    ::http::StatusCode::OK,
                    "application/json",
                    &value,
                    limits,
                ...
                GetReport200Content::OctetStream(value) => {
                    stream_response(::http::StatusCode::OK, "application/octet-stream", value)
                }
            },
            Self::BadRequest400(content) => match content {
                GetReport400Content::ProblemJson(value) => encode_json_limited(
                ...
                GetReport400Content::Json(value) => encode_json_limited(
                    ::http::StatusCode::BAD_REQUEST,
                    "application/json",
                    &value,
                    limits,
                ...
                GetReport400Content::TextPlain(value) => encode_text_limited(
                    ::http::StatusCode::BAD_REQUEST,
                    "text/plain",
                    &value,
                    limits,
...
/// Request payloads for `post_mirror` (main spec §12/§43): structured variants decode bounded; streaming variants attach the raw body verbatim; the wildcard variant carries the negotiated Content-Type.
#[derive(Debug)]
...
/// Bounded encoder for [`PostMirrorResponse`] (main spec §8 Output B, §41): JSON/text serialize under `structured_encode_bytes`; overflow discards partial output, fires the hook, and emits a fixed empty 500 (§34.1). Range/default statuses validate their carried status (§48).
impl PostMirrorResponse {
    /// Encodes the documented outcome with the configured limits.
    pub fn into_response_with_limits(
        self,
        _limits: &BodyLimits,
        ...
        match self {
            Self::Accepted202 => ::http::StatusCode::ACCEPTED.into_response(),
        }
    }
}

impl ::axum::response::IntoResponse for PostMirrorResponse {
    fn into_response(self) -> ::axum::response::Response {
        self.into_response_with_limits(&BodyLimits::process_default(), &NoOpEncodeOverflowHook)
    }
}

/// Payload for status 200 of `get_raw_text` (main spec §32): `*/*` is not a concrete media type so the application supplies the actual Content-Type (main spec §22); the body streams verbatim.
#[derive(Debug)]
...
/// Bounded encoder for [`GetRawTextResponse`] (main spec §8 Output B, §41): JSON/text serialize under `structured_encode_bytes`; overflow discards partial output, fires the hook, and emits a fixed empty 500 (§34.1). Range/default statuses validate their carried status (§48).
impl GetRawTextResponse {
    /// Encodes the documented outcome with the configured limits.
    pub fn into_response_with_limits(
        self,
        _limits: &BodyLimits,
        ...
        match self {
            Self::Ok200(wrapper) => {
                any_response(::http::StatusCode::OK, wrapper.content_type, wrapper.body)
            }
        }
    }
}

impl ::axum::response::IntoResponse for GetRawTextResponse {
    fn into_response(self) -> ::axum::response::Response {
        self.into_response_with_limits(&BodyLimits::process_default(), &NoOpEncodeOverflowHook)
    }
}

/// Documented representations for status 200 of `get_either` (main spec §11): the router selects through Content-Type matching (§28).
#[derive(Debug)]
...
/// Bounded encoder for [`GetEitherResponse`] (main spec §8 Output B, §41): JSON/text serialize under `structured_encode_bytes`; overflow discards partial output, fires the hook, and emits a fixed empty 500 (§34.1). Range/default statuses validate their carried status (§48).
impl GetEitherResponse {
    /// Encodes the documented outcome with the configured limits.
    pub fn into_response_with_limits(
        self,
        limits: &BodyLimits,
        ...
        match self {
            Self::Ok200(content) => match content {
                GetEither200Content::Json(value) => encode_json_limited(
                    ::http::StatusCode::OK,
                    "application/json",
                    &value,
                    limits,
                ...
                GetEither200Content::Any { content_type, body } => {
...
impl ::axum::response::IntoResponse for GetEitherResponse {
    fn into_response(self) -> ::axum::response::Response {
        self.into_response_with_limits(&BodyLimits::process_default(), &NoOpEncodeOverflowHook)
    }
}
...
/// Bounded encoder for [`PostStreamNoteResponse`] (main spec §8 Output B, §41): JSON/text serialize under `structured_encode_bytes`; overflow discards partial output, fires the hook, and emits a fixed empty 500 (§34.1). Range/default statuses validate their carried status (§48).
impl PostStreamNoteResponse {
    /// Encodes the documented outcome with the configured limits.
    pub fn into_response_with_limits(
        self,
        _limits: &BodyLimits,
...
impl ::axum::response::IntoResponse for PostStreamNoteResponse {
    fn into_response(self) -> ::axum::response::Response {
        self.into_response_with_limits(&BodyLimits::process_default(), &NoOpEncodeOverflowHook)
    }
}
...
pub trait Api: Send + Sync + 'static {
    /// `GET` `/reports/{id}`.
    /// Operation `getReport`.
    async fn get_report(&self, id: String) -> GetReportResponse;

    /// `POST` `/mirror`.
    /// Operation `postMirror`.
    async fn post_mirror(&self, body: PostMirrorRequestBody) -> PostMirrorResponse;

    /// `GET` `/raw-text`.
    /// Operation `getRawText`.
    async fn get_raw_text(&self) -> GetRawTextResponse;

    /// `GET` `/either`.
    /// Operation `getEither`.
    async fn get_either(&self) -> GetEitherResponse;
    ...
    async fn post_stream_note(&self, body: ::axum::body::Body) -> PostStreamNoteResponse;
}
...
/// Route handler for `GET` `/reports/{id}` (main spec §38): identity-only content coding, parameter decoding, the §28 Content-Type state machine, and bounded collection all run before the application observes the request; every failure returns a `ProtocolRejection` outside the documented enum (§39 rule 1).
async fn route_get_report(
    ...
    let response = api.get_report(id).await;
    Ok(response.into_response_with_limits(&limits, hook))
}

/// Route handler for `POST` `/mirror` (main spec §38): identity-only content coding, parameter decoding, the §28 Content-Type state machine, and bounded collection all run before the application observes the request; every failure returns a `ProtocolRejection` outside the documented enum (§39 rule 1).
async fn route_post_mirror(
    ::axum::extract::State(__state): ::axum::extract::State<ServerState>,
    ...
    let request_body = match classify_request_entry(parsed.as_ref(), &["application/json", "*/*"]) {
        ...
        RequestEntryMatch::Entry(0) => {
            ...
            let value = decode_json_body(&bytes)?;
            PostMirrorRequestBody::Json(value)
        }
        RequestEntryMatch::Entry(1) => PostMirrorRequestBody::Any {
            content_type: mime_of(parsed.as_ref()),
            body,
        },
        RequestEntryMatch::Entry(_) => unreachable!("request entry index out of range"),
    };
    let api = __state.api.as_ref();
    let response = api.post_mirror(request_body).await;
    Ok(response.into_response_with_limits(&limits, hook))
}

/// Route handler for `GET` `/raw-text` (main spec §38): identity-only content coding, parameter decoding, the §28 Content-Type state machine, and bounded collection all run before the application observes the request; every failure returns a `ProtocolRejection` outside the documented enum (§39 rule 1).
async fn route_get_raw_text(
    ::axum::extract::State(__state): ::axum::extract::State<ServerState>,
) -> Result<::axum::response::Response, ProtocolRejection> {
    let limits = __state.limits;
    let hook = __state.encode_overflow_hook.as_ref();
    let api = __state.api.as_ref();
    let response = api.get_raw_text().await;
    Ok(response.into_response_with_limits(&limits, hook))
}

/// Route handler for `GET` `/either` (main spec §38): identity-only content coding, parameter decoding, the §28 Content-Type state machine, and bounded collection all run before the application observes the request; every failure returns a `ProtocolRejection` outside the documented enum (§39 rule 1).
async fn route_get_either(
    ::axum::extract::State(__state): ::axum::extract::State<ServerState>,
) -> Result<::axum::response::Response, ProtocolRejection> {
    let limits = __state.limits;
    let hook = __state.encode_overflow_hook.as_ref();
    let api = __state.api.as_ref();
    let response = api.get_either().await;
    Ok(response.into_response_with_limits(&limits, hook))
}
...
/// Route handler for `POST` `/stream-notes` (main spec §38): identity-only content coding, parameter decoding, the §28 Content-Type state machine, and bounded collection all run before the application observes the request; every failure returns a `ProtocolRejection` outside the documented enum (§39 rule 1).
async fn route_post_stream_note(
    ::axum::extract::State(__state): ::axum::extract::State<ServerState>,
    __headers: ::http::HeaderMap,
    body: ::axum::body::Body,
) -> Result<::axum::response::Response, ProtocolRejection> {
    ...
    let api = __state.api.as_ref();
    let response = api.post_stream_note(request_body).await;
    Ok(response.into_response_with_limits(&limits, hook))
}
...
pub fn router(
    ...
    ::axum::Router::new()
        .route("/reports/{id}", ::axum::routing::get(route_get_report))
        .route("/mirror", ::axum::routing::post(route_post_mirror))
        .route("/raw-text", ::axum::routing::get(route_get_raw_text))
        .route("/either", ::axum::routing::get(route_get_either))
        .route(
            "/notes/{id}",
        ...
        .route(
            "/stream-notes",
            ::axum::routing::post(route_post_stream_note),
        )
        .with_state(state)
}
...
/// Bounded plain-text response encoding (§34/§41).
fn encode_text_limited(
    status: ::http::StatusCode,
    content_type: &'static str,
    value: &str,
    limits: &BodyLimits,
