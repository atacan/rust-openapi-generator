...
/// Sequential semantics (§51.4): while this part is open the rest of the message cannot advance. After it ends, trailing parts keep flowing through the framing engine: a duplicate declared single-valued part rejects, every observed name is recorded for `unknown_part_names`, and all remaining payloads drain without buffering.
pub struct UploadDocumentFilePart {
    pub file_name: Option<String>,
    pub content_type: Option<::mime::Mime>,
    events: MultipartEvents,
    log: ::std::sync::Arc<::std::sync::Mutex<MultipartUnknownLog>>,
    stage: MultipartTailStage,
    finished: bool,
}

impl UploadDocumentFilePart {
    /// Advances to the next payload chunk of this part (`None` at its end). Violations encountered while draining trailing parts surface here as protocol rejections because sequential streaming cannot validate them any earlier.
        ...
        loop {
            ...
            match event {
                ...
                Ok(MultipartEvent::PartBegin(headers)) => {
                    if matches!(headers.name.as_str(), "metadata" | "file") {
                        return Err(schema_violation(format!(
                            "duplicate single-valued part `{}` after the streaming part",
                            headers.name
                        )));
                    }
                    multipart_record_unknown(&self.log, &headers.name);
                    self.stage = MultipartTailStage::Drain;
                }
                Ok(MultipartEvent::PartChunk(chunk)) => {
                    if matches!(self.stage, MultipartTailStage::Idle) {
                        return Ok(Some(chunk));
                    }
                }
                Ok(MultipartEvent::PartEnd) => {
                    self.stage = MultipartTailStage::Idle;
                }
            }
        }
    }
...
/// Streaming multipart input for `upload_document` (main spec §17 Output B): scalar/JSON parts were bounded-buffered and decoded during the router's single incremental pass; binary parts stay live streams over the request body.
/// Required-part enforcement ran inside that pass, so missing required parts reject 422 BEFORE the application handler runs (§17.1). Planning requires required scalar/JSON parts to be declared before binary parts, matching what sequential streaming can validate up front.
...
/// Runs the §38 pre-handler pipeline for the `multipart/form-data` body of `upload_document` (main spec §5.5/§17/§17.1): one incremental pass; scalar/JSON parts buffer only up to `multipart_scalar_part_bytes`; required and duplicate-single-valued parts reject before the trait runs; binary parts hand off as live streams.
#[allow(clippy::missing_errors_doc)]
...
/// Tail-scan phase of a streaming part: `Idle` delivers payload chunks to the application; `Drain` discards trailing payloads (post-handoff parts) while their names stay observable.
#[derive(Debug, Clone, Copy)]
enum MultipartTailStage {
    Idle,
    Drain,
}
...
#[allow(clippy::needless_pass_by_ref_data)]
    ...
    T: ::std::str::FromStr,
{
    ...
    text.parse().map_err(|_| {
        schema_violation(format!("part `{wire}` failed schema validation"))
    })
}
