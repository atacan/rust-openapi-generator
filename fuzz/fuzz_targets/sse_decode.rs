//! §50 test 41 fuzz target: SSE framing + JSON decode (main spec §18.2).
//!
//! Invariants over each input:
//! - fed as ONE chunk and as deterministic 1–7-byte chunks;
//! - at generous (4096) and tight (64) per-event limits so
...
//!   declared `SseDecodeError` variants followed by a clean `None`.
...
fuzz_target!(|data: &[u8]| {
    ...
    for parts in chunkings {
        for limit in [4096_usize, 64] {
            let stream = decode_sse_json::<serde_json::Value, _, _>(
